...
bool QuestionRepo::removeToReview(Question p) {
	int poz = findPozProductReview(p);
	if (poz != -1) {
		for (int i = poz; i < review.size() - 1; i++) {
			review[i] = review[i + 1];
		}
		review.pop_back();
		return true;
...
int QuestionRepo::displayBySubject(char* subject)
{
	int i, j,numberOfQuestions=0;
	if (elems.size() == -1)
	...
	else
	{
		for (i = 0;i <= elems.size() - 1;i++)
		{
			if (strcmp(elems[i].getSubject(), subject) == 0)
			{
				printf("%s", elems[i].getSubject());
				elems[i].toStringStudent();
				numberOfQuestions++;
			}
		}
